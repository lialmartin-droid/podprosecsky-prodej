/**
 * Podprosečské domácí produkty — sdílený backend V9
...
function readProducts_() {
  const sheet = getOrCreateSheet_(CONFIG.PRODUCTS_SHEET);
  formatProductsSheet_(sheet); seedProducts_(sheet); repairDefaultProductSettings_(sheet);
  ...
  return rows.filter(r => r[0] !== '').map(r => ({
    ...
    leadDays: Number(r[10] || 0), quick: quickButtonsForProduct_(r[0], r[1], r[2], r[11])
  }));
}
...
function normalizeProduct_(p) {
  const id = cleanText_(p.id,100) || Utilities.getUuid();
  const emoji = cleanText_(p.emoji || '📦',10);
  const name = cleanText_(p.name,100);
  return {
    id: id, emoji: emoji,
    name: name, price: Math.max(0,Number(p.price)||0), unit: cleanText_(p.unit || 'kus',30),
    ...
    quick: quickButtonsForProduct_(id, emoji, name, p.quick)
  };
}
...
function quickButtonsForProduct_(id, emoji, name, value) {
  // Vejce mají v projektu stálé ID 2. U nich se rychlá tlačítka vrací vždy,
  // bez ohledu na prázdnou nebo starou hodnotu v tabulce.
  if (String(id) === '2') return [6, 10, 30];

  const text = `${emoji || ''} ${name || ''}`.toLocaleLowerCase('cs-CZ');
  if (text.indexOf('🥚') !== -1 || text.indexOf('vejce') !== -1) return [6, 10, 30];

  return String(value || '').split(',').map(x => Number(x.trim())).filter(x => Number.isFinite(x) && x > 0);
}

function repairDefaultProductSettings_(sheet) {
  const values = sheet.getDataRange().getValues();
  for (let row = 1; row < values.length; row++) {
    const quick = quickButtonsForProduct_(values[row][0], values[row][1], values[row][2], values[row][11]);
    const isEggProduct = quick.length && !String(values[row][11] || '').trim();
    if (isEggProduct) {
      sheet.getRange(row + 1, 12).setValue(quick.join(', '));
      sheet.getRange(row + 1, 13).setValue(new Date());
    }
  }
}
...
function htmlResponse_(ok,message,id,extra) { return HtmlService.createHtmlOutput(`<!doctype html><meta charset="utf-8"><script>window.parent.postMessage(${JSON.stringify(Object.assign({type:'PDP_BACKEND_RESULT',ok:ok,message:message,id:id},extra||{}))},'*');<\/script>`).setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL); }
