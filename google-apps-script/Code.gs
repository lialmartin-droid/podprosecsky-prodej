...
function repairDefaultProductSettings_(sheet) {
  const props = PropertiesService.getScriptProperties();
  if (props.getProperty('MIGRATION_V5_QUICK_BUTTONS') === 'done') return;

  const values = sheet.getDataRange().getValues();
  for (let row = 1; row < values.length; row++) {
    if (String(values[row][0]) === '2' && !String(values[row][11] || '').trim()) {
      sheet.getRange(row + 1, 12).setValue('6, 10, 30');
      sheet.getRange(row + 1, 13).setValue(new Date());
      break;
    }
  }

  props.setProperty('MIGRATION_V5_QUICK_BUTTONS', 'done');
}
