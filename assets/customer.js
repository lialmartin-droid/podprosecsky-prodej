window.PDP_CUSTOMER_VERSION = "12";
console.info("Podprosečské produkty – customer.js V12");
...
function calculatePickupMinimum() {
  ...
  return {
    ...
    blocked: false,
    message: `Pro ${eggs} vajec je nejbližší možný termín vyzvednutí ${localDate(earliest.date)}.`
  };
}
...
function updatePickupAvailability() {
  ...
  if (result.message) {
    ...
    availabilityEl.classList.remove("hidden", "availability-error");
    if (result.blocked) availabilityEl.classList.add("availability-error");
  } else {
    ...
    article.innerHTML = `
      <div class="product-row">
        ...
        <div style="flex:1">
          ...
          ${isEggProduct(product) ? `<div class="notice">Termín vyzvednutí vajec se automaticky vypočítá podle aktuálního skladu, denní snášky a již přijatých objednávek.</div>` : ""}
